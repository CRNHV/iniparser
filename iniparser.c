...
void trim(char* str, int* begin_ind, int* end_ind)
{
	int b = *begin_ind;
	int e = *end_ind;
	int i;
...
int read_line(struct read_ini* read_ini)
{
	int i = 0;
	int b;
	int new = 0;
...
struct section* parse_section(struct read_ini* read_ini)
{
	int new;
		...
		if (new)
		{
			...
			if (read_ini->tmp[b] == '[')
			{
				...
				else
				{
					/* fill section name */
					read_ini->tmp[e] = '\0';
					section->name = pMalloc(e);
					memset(section->name, 0, e);
					memcpy(section->name, &read_ini->tmp[b + 1], e - 1);
			...
			cfg->key = malloc(y + 2);
			memset(cfg->key, 0, y + 2);
			memcpy(cfg->key, &read_ini->tmp[x], y + 2);
			...
			cfg->value = pMalloc(y + 1);
			memset(cfg->value, 0, y);
			memcpy(cfg->value, &read_ini->tmp[x], y + 1);
			...
			{
				section->num_configs++;
				section->configs = realloc(section->configs, sizeof(*section->configs) * (section->num_configs * 2));
			...
			{
				section->configs = malloc(sizeof(*section->configs) * 2);
...
struct ini* parse_ini(struct read_ini* read_ini)
{
...
struct ini*	read_ini(struct read_ini** read_inip,char* ini_str, size_t ini_str_len)
{
	...
	read_ini->ini_str = ini_str;
	read_ini->ini_str_size = ini_str_len;
...
char* ini_get_value(struct ini* ini,
	char* section,
	char* key)
{
	int s, c;
